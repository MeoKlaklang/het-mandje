...
export async function getAsielDashboardData() {
  ...
  const { data: shelter, error: shelterError } = await supabase
    .from("shelters")
    .select("id, name, description, owner_id, city")
    .eq("owner_id", user.id)
    .single();
  ...
  const { data: animals, error: animalsError } = await supabase
    .from("animals")
    .select(
      "id, name, species, breed, age, gender, size, status, image_url, created_at"
    )
  ...
  const applicationsList =
    (applications || []) as AsielDashboardApplication[];
  ...
  const dashboardData: AsielDashboardData = {
    ...
    availableAnimals: animalsList.filter(
      (animal) => animal.status === "beschikbaar"
    ).length,
    pendingApplications: applicationsList.filter(
      (application) => application.status === "in_afwachting"
    ).length,
  };

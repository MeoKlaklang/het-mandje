...
export default function DierDetailPage() {
  ...
  useEffect(() => {
    async function loadAnimal() {
      ...
      if (animal?.available_from) {
        setCalendarMonth(new Date(animal.available_from));
      }
  ...
  const startDate = animal.available_from
    ? new Date(animal.available_from)
    : null;
  const endDate = animal.available_until
    ? new Date(animal.available_until)
    : null;

  const shelterWebsite = normalizeWebsiteUrl(shelter?.website);
  const shelterEmail = shelter?.email ? `mailto:${shelter.email}` : null;
  const contactLink = shelterWebsite || shelterEmail;
      ...
      <div className={styles.container}>
        <section className={styles.leftColumn}>
          ...
          <section className={styles.descriptionSection}>
            ...
            <p>
              {animal.description ||
                "Er is nog geen uitgebreide beschrijving beschikbaar voor dit dier."}
            </p>
        ...
        <aside className={styles.rightColumn}>
          ...
          {images[4] && (
            <div className={styles.shelterImage}>
              <img src={images[4]} alt={`${animal.name} extra foto`} />
            </div>
          )}
          ...
          <div className={styles.contactCard}>
            ...
            {contactLink ? (
              <a
                href={contactLink}
                target={shelterWebsite ? "_blank" : undefined}
                rel={shelterWebsite ? "noopener noreferrer" : undefined}

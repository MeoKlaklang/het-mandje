...
import { getAsielDashboardData, AsielDashboardAnimal, AsielDashboardData } from "@/lib/asiel/getAsielDashboardData";
...
const tasks = ["Quarantaine ruimte schoonmaken van de katten", "Nieuwe aanvragen controleren", "Medicatieoverzicht nakijken"];
...
function getApplicationsForAnimal(animalId: string, data: AsielDashboardData) {
	return data.applications.filter((application) => application.animal_id === animalId);
}
		...
		<main className={styles.page}>
			<aside className={styles.sidebar}>
				<Link href="/home" className={styles.logoLink}>
					<Image src="/images/logo.png" alt="Het Mandje logo" width={90} height={65} className={styles.logo} />
				</Link>
			...
			<section className={styles.content}>
				<header className={styles.topbar}>
					<div className={styles.searchBar}>
						<input type="text" placeholder="Zoek een dier, chipnummer, adoptant" />
						<span>⌕</span>
					</div>
							...
							<section className={styles.mainGrid}>
									...
									<article className={styles.animalOverview}>
										...
										{recentAnimals.length === 0 ? (
											<div className={styles.emptyAnimals}>
												...
												<p>Voeg je eerste hond of kat toe zodat pleeggezinnen interesse kunnen tonen en je aanvragen kan opvolgen.</p>
														...
														<article key={animal.id} className={styles.animalCard}>
															...
															<div className={styles.animalCardInfo}>
																<div className={styles.animalCardTitle}>
																	...
																	<span className={`${styles.statusBadge} ${getStatusClass(animal.status)}`}>{formatStatus(animal.status)}</span>
																</div>
								...
								<aside className={styles.rightColumn}>
									...
									<article className={styles.notificationCard}>
										...
										{dashboardData.pendingApplications > 0 ? <p>Nieuwe aanvraag voor een dier uit jouw asiel.</p> : <p>Er zijn momenteel geen nieuwe aanvragen.</p>}
									</article>
								</aside>
							</section>
						</>

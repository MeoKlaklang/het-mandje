...
import { getAsielDashboardData, AsielDashboardAnimal, AsielDashboardData, AsielDashboardApplication } from "@/lib/asiel/getAsielDashboardData";
import { getApplicationDetailsForShelter, ApplicationDetails } from "@/lib/asiel/getApplicationDetailsForShelter";
import { updateApplicationStatus } from "@/lib/asiel/updateApplicationStatus";
import { getShelterAgendaData, ShelterAgendaAppointment } from "@/lib/asiel/getShelterAgendaData";
...
function getApplicationsForAnimal(animalId: string, data: AsielDashboardData) {
	return data.applications.filter((application) => application.animal_id === animalId && application.status === "in_afwachting");
}
...
function getAnimalName(animalId: string | null, animals: AsielDashboardAnimal[]) {
...
export default function AsielDashboardPage() {
	const [dashboardData, setDashboardData] = useState<AsielDashboardData | null>(null);
	...
	async function loadDashboard() {
		...
		const [dashboardResult, agendaResult, todoResult] = await Promise.all([getAsielDashboardData(), getShelterAgendaData(), getShelterTodos()]);
	...
	const availableAnimals = useMemo(() => {
		...
		return dashboardData.animals.filter((animal) => animal.status === "beschikbaar").slice(0, 4);
	}, [dashboardData]);

	const aanvraagAnimals = useMemo(() => {
		if (!dashboardData) return [];

		const animalIdsWithApplications = new Set(dashboardData.applications.filter((application) => application.status === "in_afwachting").map((application) => application.animal_id));

		return dashboardData.animals.filter((animal) => animalIdsWithApplications.has(animal.id)).slice(0, 4);
	}, [dashboardData]);

	const fosterAnimals = useMemo(() => {
		if (!dashboardData) return [];

		return dashboardData.animals.filter((animal) => animal.status === "in_opvang").slice(0, 4);
	}, [dashboardData]);
	...
	}, [activeAnimalTab, availableAnimals, aanvraagAnimals, fosterAnimals, recentAnimals]);
	...
	const recentUpdates = useMemo(() => {
		...
		const applicationUpdates = dashboardData.applications.map((application) => ({
			...
			title: `Nieuwe aanvraag voor ${getAnimalName(application.animal_id, dashboardData.animals)}`,
	...
	const handleOpenApplication = async (applicationId: string) => {
		...
		const { application, error } = await getApplicationDetailsForShelter(applicationId);
	...
	const handleDecision = async (decision: "goedgekeurd" | "afgewezen") => {
		...
		const confirmed = confirm(decision === "goedgekeurd" ? "Weet je zeker dat je deze aanvraag wilt goedkeuren?" : "Weet je zeker dat je deze aanvraag wilt weigeren?");
		...
		alert(decision === "goedgekeurd" ? "Aanvraag werd goedgekeurd." : "Aanvraag werd geweigerd.");
	...
	return (
		<AsielLayout>
					...
					<>
						...
						<section className={styles.statsGrid}>
							<article className={styles.statCard}>
								...
								<span>
									{dashboardData.animals.filter((animal) => animal.species === "hond").length} honden &nbsp;&nbsp;
									{dashboardData.animals.filter((animal) => animal.species === "kat").length} katten
								</span>
								...
								<article className={styles.animalOverview}>
									<div className={styles.overviewHeader}>
										<div>
											...
											<p>Bekijk welke dieren zichtbaar zijn en waar aanvragen op lopen.</p>
										</div>
									...
									<div className={styles.tabs}>
										{animalTabs.map((tab) => (
											<button key={tab.id} type="button" className={activeAnimalTab === tab.id ? styles.activeTab : ""} onClick={() => setActiveAnimalTab(tab.id)}>
												{tab.label}
											</button>
										))}
									</div>
									...
									{visibleAnimals.length === 0 ? (
										<div className={styles.emptyAnimals}>
											...
											<p>Er zijn momenteel geen dieren binnen deze categorie.</p>

											<Link href="/asiel/dieren/nieuw">+ Nieuw dier toevoegen</Link>
										</div>
									) : (
										<div className={styles.animalCardsList}>
											{visibleAnimals.map((animal: AsielDashboardAnimal) => {
												const applications = getApplicationsForAnimal(animal.id, dashboardData);

												const firstPendingApplication = applications.find((application: AsielDashboardApplication) => application.status === "in_afwachting");

												return (
													<article key={animal.id} className={styles.animalCard}>
														<Link href={`/asiel/dieren/${animal.id}/dossier`}>
															<img src={animal.image_url || "/images/dog3.jpg"} alt={animal.name} className={styles.animalCardImage} />
														</Link>

														<div className={styles.animalCardInfo}>
															<div className={styles.animalCardTitle}>
																<Link href={`/asiel/dieren/${animal.id}/dossier`} className={styles.animalNameLink}>
																	<h3>{animal.name}</h3>
																</Link>

																<span className={`${styles.statusBadge} ${getStatusClass(animal.status)}`}>{activeAnimalTab === "aanvragen" ? "Aanvraag" : formatStatus(animal.status)}</span>
															</div>
															...
															<p className={styles.animalDate}>{formatDate(animal.created_at)}</p>

															<div className={styles.animalCardFooter}>
																{firstPendingApplication ? (
																	<button type="button" className={styles.applicationButton} onClick={() => handleOpenApplication(firstPendingApplication.id)}>
																		⏳ {applications.length} {applications.length === 1 ? "aanvraag" : "aanvragen"}
																	</button>
																) : (
																	<span className={styles.noApplicationBadge}>Geen aanvragen</span>
																)}

																<Link href={`/asiel/dieren/${animal.id}/bewerken`} className={styles.viewAnimalLink}>
																	Bewerk →
																</Link>
															</div>
														</div>
							...
							<aside className={styles.rightColumn}>
									...
									<div className={styles.miniCalendar}>
										...
										<div className={styles.calendarGrid}>
											{calendarDays.map((day) => {
												const hasAppointment = appointments.some((appointment) => sameDay(new Date(appointment.start_at), day));
								...
								<article className={styles.sideCard}>
									...
									<div className={styles.todoInput}>
										...
										<button type="button" disabled={todoSaving} onClick={handleCreateTodo}>
											+
										</button>
									</div>
									...
									<div className={styles.taskList}>
										{todos.length === 0 ? (
											<p className={styles.emptyTodo}>Geen todo’s voor vandaag.</p>
										) : (
											todos.map((todo) => (
												<label key={todo.id} className={`${styles.taskItem} ${todo.is_done ? styles.taskDone : ""}`}>
									...
									<p className={styles.todoHint}>Afgevinkte todo’s verdwijnen automatisch de volgende dag.</p>
								</article>
			...
			{selectedApplication && (
				<div className={styles.modalOverlay}>
					<div className={styles.applicationModal}>
						<button type="button" className={styles.closeModal} onClick={handleCloseModal}>
							×
						</button>

						<div className={styles.modalHeader}>
							<p>Aanvraag bekijken</p>
							<h2>Aanvraag voor {selectedApplication.animal?.name || "dit dier"}</h2>
						</div>
						...
						<div className={styles.modalGrid}>
							<section className={styles.modalCard}>
								...
								<div className={styles.modalAnimal}>
									<img src={selectedApplication.animal?.image_url || "/images/dog3.jpg"} alt={selectedApplication.animal?.name || "Dier"} />

									<div>
										<h4>{selectedApplication.animal?.name}</h4>
										<p>
											{selectedApplication.animal?.breed || selectedApplication.animal?.species} · {selectedApplication.animal?.age || "Leeftijd onbekend"}
										</p>
										<span>Status: {formatStatus(selectedApplication.animal?.status || null)}</span>
									</div>
								</div>
							</section>
							...
							<section className={styles.modalCard}>
								...
								<p>Geboortedatum: {formatNormalDate(selectedApplication.profile?.birth || null)}</p>

								<p>Adres: {[selectedApplication.profile?.street, selectedApplication.profile?.house_number, selectedApplication.profile?.postal_code, selectedApplication.profile?.city].filter(Boolean).join(" ") || "Niet ingevuld"}</p>
							</section>
						</div>
						...
						<section className={styles.modalCard}>
							...
							<div className={styles.infoRows}>
								<p>
									<strong>Periode:</strong> {formatNormalDate(selectedApplication.start_date)} - {formatNormalDate(selectedApplication.end_date)}
								</p>

								<p>
									<strong>Status:</strong> {selectedApplication.status || "Onbekend"}
								</p>

								<p>
									<strong>Bericht:</strong> {selectedApplication.message || "Geen bericht toegevoegd."}
								</p>
							</div>
						</section>
						...
						<section className={styles.modalCard}>
							...
							{selectedApplication.fosterApplication?.answers ? (
								<div className={styles.answersList}>
									{Object.entries(selectedApplication.fosterApplication.answers).map(([key, value]) => (
						...
						<div className={styles.modalActions}>
							<button type="button" className={styles.rejectButton} disabled={decisionLoading} onClick={() => handleDecision("afgewezen")}>
								{decisionLoading ? "Bezig..." : "Weigeren"}
							</button>

							<button type="button" className={styles.approveButton} disabled={decisionLoading} onClick={() => handleDecision("goedgekeurd")}>

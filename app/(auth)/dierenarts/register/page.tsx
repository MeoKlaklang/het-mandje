...
import { getSheltersForVeterinarian, ShelterOption } from "@/lib/dierenarts/getSheltersForVeterinarian";
...
export default function DierenartsRegisterPage() {
	...
	const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (signUpError || !signUpData.user) {
			setLoading(false);
			setErrorMessage(signUpError?.message || "Account kon niet aangemaakt worden.");
			return;
		}
		...
		const { error: veterinarianError } = await supabase.from("veterinarians").insert({
			user_id: userId,
		...
		<main className={styles.page}>
			<div className={styles.topBar}>
				<Link href="/" className={styles.logoLink}>
					<img src="/images/LOGO.png" alt="Het Mandje logo" className={styles.logoImage} />
				</Link>
			...
			<div className={styles.wrapper}>
				<section className={styles.intro}>
					...
					<p>Koppel je praktijk aan een dierenasiel en beheer medische dossiers, medicatie en afspraken centraal binnen Het Mandje.</p>
				</section>
				...
				<form onSubmit={handleRegister} className={styles.formCard}>
					<section className={styles.formSection}>
						...
						<div className={styles.gridTwo}>
							<label>
								E-mailadres
								<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="naam@praktijk.be" />
							</label>

							<label>
								Wachtwoord
								<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Minstens 6 tekens" />
							</label>
						</div>
					</section>
					...
					<section className={styles.formSection}>
						...
						<label>
							Praktijknaam
							<input type="text" value={practiceName} onChange={(e) => setPracticeName(e.target.value)} placeholder="Bijv. Dierenartspraktijk Kingen" />
						</label>

						<div className={styles.gridTwo}>
							<label>
								Dierenartsnummer
								<input type="text" value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)} placeholder="Erkenningsnummer" />
							</label>
						...
						<div className={styles.gridTwo}>
							<label>
								Straat
								<input type="text" value={street} onChange={(e) => setStreet(e.target.value)} placeholder="Straat" />
							</label>

							<label>
								Huisnummer
								<input type="text" value={houseNumber} onChange={(e) => setHouseNumber(e.target.value)} placeholder="12" />
							</label>
						</div>
						...
						<div className={styles.gridTwo}>
							<label>
								Postcode
								<input type="text" value={postalCode} onChange={(e) => setPostalCode(e.target.value)} placeholder="2800" />
							</label>

							<label>
								Gemeente
								<input type="text" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Mechelen" />
							</label>
						</div>
					</section>
					...
					<section className={styles.formSection}>
						...
						<label>
							...
							<input
								...
								placeholder={loadingShelters ? "Dierenasielen laden..." : "Zoek op naam of gemeente"}
							/>
						</label>
					...
					<div className={styles.actions}>
						<p>
							Heb je al een account? <Link href="/dierenarts/login">Log in</Link>
						</p>
